...
public sealed class MCH_Default : MachinistRotation
{
    ...
    protected override bool AttackAbility(IAction nextGCD, out IAction? act)
    {
        ...
        if (IsBurst && BSPrio && BarrelStabilizerPvE.CanUse(out act))
        {
            return true;
        }
        ...
        if (IsBurst)
        {
            ...
            {
                if (WeaponRemain < 1.25f && WildfirePvE.CanUse(out act))
                {
                    return true;
                }
        ...
        {
            if ((!HoldHCForCombo || !(LiveComboTime <= 8f && LiveComboTime > 0f)) && ToolChargeSoon(out act))
            {
                return true;
            }
        }
        ...
        if (DumbQueen && InCombat && RookAutoturretPvE.CanUse(out act, skipTTKCheck: true))
        {
            return true;
        }
        ...
        if (isRicochetMore && RicochetPvE.CanUse(out act, usedUp: true))
        {
            return true;
        }
    ...
    {
        if (AirAnchorPvE.Cooldown.WillHaveOneChargeGCD(1) && BarrelStabilizerPvE.Cooldown.WillHaveOneChargeGCD(6) && WildfirePvE.Cooldown.WillHaveOneChargeGCD(6))
        {
