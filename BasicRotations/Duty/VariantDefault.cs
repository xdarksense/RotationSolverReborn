...
internal class VariantDefault : VariantRotation
{
    ...
    {
        if (VariantCurePvE.CanUse(out act, skipSelfStatusProvideCheck: true)) return true;
        if (VariantCurePvE_33862.CanUse(out act, skipSelfStatusProvideCheck: true)) return true;
